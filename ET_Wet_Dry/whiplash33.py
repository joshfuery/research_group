...
Whiplash analysis using 33rd/66th percentile thresholds on MODIS ET data.
...
Outputs saved to: api/figures/whiplash33/
"""
...
# ---------------------------------------------------------------------------
# Basin → ET CSV filename stem
# ---------------------------------------------------------------------------
BASINS = {
    "Amazon Basin":              ET_DIR / "MODIS_ET_Amazon_2002_2024.csv",
    "California":                ET_DIR / "MODIS_ET_California_2002_2024.csv",
    "Colorado":                  ET_DIR / "MODIS_ET_Colorado_2002_2024.csv",
    "Congo":                     ET_DIR / "MODIS_ET_Congo_2002_2024.csv",
    "Danube":                    ET_DIR / "MODIS_ET_Danube_2002_2024.csv",
    "Ganges / Brahmaputra Basin": ET_DIR / "MODIS_ET_Ganga-Brahmaputra_2002_2024.csv",
    "Mekong Basin":              ET_DIR / "MODIS_ET_Mekong_2002_2024.csv",
    "Mississippi / Missouri":    ET_DIR / "MODIS_ET_Mississippi_2002_2024.csv",
    "Murray Darling":            ET_DIR / "MODIS_ET_Murray_Darling_2002_2024.csv",
    "Nile":                      ET_DIR / "MODIS_ET_Nile_2002_2024.csv",
    "Yangtze Basin":             ET_DIR / "MODIS_ET_Yangtze_2002_2024.csv",
}
...
def load_basin(basin: str) -> pd.Series:
    csv_path = BASINS[basin]
    df = pd.read_csv(csv_path)
    df["date"] = pd.to_datetime(
        dict(year=df["year"], month=df["month"], day=1)
    )
    df = df.sort_values("date").set_index("date")
    series = pd.to_numeric(df["ET_mm"], errors="coerce").dropna()
    return series
...
def plot_stats_timeseries(basin: str, series: pd.Series, anomaly: pd.Series) -> None:
    # Rolling stats on raw ET — 12-month lookback window for all three
...
def plot_combined_figure(basin: str, series: pd.Series, anomaly: pd.Series) -> None:
    ...
    axes[7].set_ylabel("ET Anomaly (mm)")
    axes[7].set_xlabel("Date")
    axes[7].legend(["Monthly ET Anomaly"], loc="upper right", fontsize=9)
