...
The exact same methodology runs across two independent dimensions:
  ...
  - Data source (see DATA_SOURCES): MODIS ET, and GPM IMERG precipitation.
Nothing about the modeling approach changes across any of these four
combinations — only the time series being fed into it.

For each basin, at each resolution/source combination:
...
Data sources:
  ET:     api/csv/2002-2024/MODIS_ET_*_2002_2024.csv
  Precip: api/csv/Figure3A/imerg_basin_monthly_timeseries.csv (GPM IMERG)

Outputs saved to: api/figures/whiplash33/ML/
  monthly/    analysis at Monthly anomaly-block resolution
    ET/         MODIS ET whiplash analysis
    precip/     GPM IMERG precipitation whiplash analysis
  3_month/    identical methodology, at 3-Month anomaly-block resolution
    ET/
    precip/
  each ET/ or precip/ folder contains:
...
# Precipitation source: basin-averaged monthly GPM IMERG, already computed
# for the Fig 3a P-ET-R figures. Column names differ slightly from
# whiplash33.BASINS for two basins — mapped explicitly below.
PRECIP_CSV = Path(__file__).resolve().parent / "csv" / "Figure3A" / "imerg_basin_monthly_timeseries.csv"
PRECIP_BASIN_MAP = {
...
def load_basin_precip(basin: str) -> pd.Series:
    ...
    series = pd.to_numeric(df[PRECIP_BASIN_MAP[basin]], errors="coerce").dropna()
    series.index.name = None
    return series


# Same methodology, run once per data source. `loader(basin)` returns the
# raw monthly Series for that basin; everything downstream (anomaly,
# whiplash detection, features, models) is identical either way.
DATA_SOURCES = [
    {"label": "ET", "dirname": "ET", "loader": w33.load_basin},
    {"label": "Precipitation", "dirname": "precip", "loader": load_basin_precip},
]
...
def get_block_series(basin: str, rule: str | None, loader) -> tuple[pd.Series, pd.Series]:
    """Return (series, anomaly) at the requested block resolution, for the
    given data-source loader. The anomaly is always computed from the
    Monthly climatology first (so the "extreme month" comparison stays
    meaningful), then both series and anomaly are summed into blocks —
    exactly how whiplash33.py's WINDOW_CONFIGS aggregate the Monthly
    anomaly into 3-/6-Month blocks."""
    series = loader(basin)
...
def build_features(series: pd.Series, anomaly: pd.Series) -> pd.DataFrame:
    ...
    is_whiplash, p33, p66 = w33.detect_whiplash_33(anomaly)
    bar_state = pd.Series(
...
def run_block_config(block_label: str, rule: str | None, block_dirname: str,
                      source_label: str, loader, source_dirname: str) -> None:
    dirs = make_dirs(ML_DIR / block_dirname / source_dirname)
    print(f"=== {block_label} block resolution — {source_label} — "
    ...
    for basin in w33.BASINS:
        ...
        try:
            series, anomaly = get_block_series(basin, rule, loader)
...
def main():
    for block in BLOCK_CONFIGS:
        for source in DATA_SOURCES:
            run_block_config(
                block["label"], block["rule"], block["dirname"],
                source["label"], source["loader"], source["dirname"],
            )

    print("All block resolution / data source combinations done.")
