...
# Precipitation source: basin-averaged monthly GPM IMERG from this
# workspace. whiplash33 has already been adapted to read precip, so its
# load_basin IS the precip loader here.
load_basin_precip = w33.load_basin


# PRECIPITATION RERUN: the upstream script also ran a MODIS ET source, but
# this workspace has no per-basin ET CSVs, so only the precip source runs.
DATA_SOURCES = [
